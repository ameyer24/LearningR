...
cost.table.1 <- function(DatabaseName,StartYear,EndYear){
  Tidy_DB_Pricing %>%
    ...
    arrange(Fiscal_Year) %>%
    group_by(Database) %>%
    mutate(Price_Change = Cost-lag(Cost)) %>%
    mutate(Percent_Change = (Cost-lag(Cost))/lag(Cost)) %>%
    mutate(Percent_Change = paste(round((Percent_Change * 100), digits=2),"%",sep="")) %>%
...
### Cost
This is an introduction to the cost section.
```{r}
dbName = "Business Source Complete"
kable(cost.table.1(dbName, 2012, 2018))
cost.graph.1(dbName)
```
\newpage
